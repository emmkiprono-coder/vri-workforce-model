    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <Panel>
          ...
          <SliderRow label="Monthly call volume target" value={state.monthlyVol} display={state.monthlyVol.toLocaleString()} min={500} max={100000} step={100} onChange={v => update({ monthlyVol: v })} />
          <SliderRow label="Peak hour concentration (% in 4h)" value={state.peakPct} display={`${state.peakPct}%`} min={20} max={80} step={1} onChange={v => update({ peakPct: v })} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        <MetricCard label="Available handle time/FTE/day" value={fmt(availAfterShrink, 0) + ' min'} sub="After shrinkage" />
        <MetricCard label="Max calls/FTE/day (capacity)" value={fmt(maxCalls, 1)} sub="At current AHT" />
        <MetricCard label="FTEs required for monthly volume" value={fmt(fteReq, 0) + ' FTEs'} variant="info" sub="Based on all parameters" />
        <MetricCard label="Billable minutes/FTE/day" value={fmt(billablePerFTE, 0) + ' min'} sub="Talk time only" />
      </div>
        ...
        <table>
          ...
          <tbody>
            {ahtRange.map(a => {
              ...
              const isCurrent = Math.abs(a - state.aht) < 0.5
              return (
                <tr key={a} className={isCurrent ? 'highlight-row' : ''}>
                  <td className={isCurrent ? 'font-semibold text-[#00D4A0]' : ''}>{a} min{isCurrent ? ' ← current' : ''}</td>
                  <td>{fmt(callsN, 1)}</td>
                  <td>{fmt(billablePerFTE, 0)}</td>
                  <td>{fmt(fteN, 0)}</td>
                  <td>{fmtM(cpm, 3)}</td>
                  <td style={{ color: diff <= 0 ? '#34d399' : '#f87171' }}>{(diff >= 0 ? '+' : '') + fmtM(diff, 3)}</td>
                  <td><Badge variant={diff <= 0 ? 'success' : 'danger'}>{diff <= 0 ? 'Competitive' : 'Above vendor'}</Badge></td>
                </tr>
              )
            })}
          </tbody>
